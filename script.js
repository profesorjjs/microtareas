...
function setSubmitProgress(done, total, label) {
  ...
  if (btn) {
    btn.disabled = true;
    btn.textContent = `Enviando… (${done}/${total})`;
  }

  if (msgBox) {
    msgBox.className = "message note";
    msgBox.innerHTML = `
      <div class="send-progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${pct}">
        <div class="send-progress-bar" style="width:${pct}%"></div>
      </div>
      <div class="send-progress-text">${label || "Enviando…"}</div>
    `;
  }
}
...
window.addEventListener("load", () => {
  try {
    ...
    setSubmitState(false, \"Ha ocurrido un error al enviar. Revisa tu conexión y vuelve a intentarlo.\");
    console.error(err);
  }
});
});
...
submitAllBtn?.addEventListener("click", async () => {
  if (window.__sendingAll) return; // evita doble clic
  window.__sendingAll = true;

  setSubmitProgress(0, 6, "Preparando el envío…");
  ...
  try {
    setSubmitProgress(1, 6, "Validando formularios…");
  const msg = document.getElementById("wizard-message");
  if (msg) {
    msg.textContent = "";
    msg.className = "message";
...
// --- Preparar imágenes y análisis IA (por microtarea) ---
    ...
    ])    setSubmitProgress(3, 6, "Guardando datos…");
;
    ...
    await addDoc(photosCol, {
      ...
      aiScore: mt1.aiScore,
    setSubmitProgress(4, 6, "Subiendo fotografías (2/3)…");
    ...
    await addDoc(photosCol, {
      ...
      taskId: "MT2_ESCOLAR",
      dataUrl: mt2.dataUrl,
    setSubmitProgress(4, 6, "Subiendo fotografías (3/3)…");
    setSubmitProgress(5, 6, "Cerrando y finalizando…");
aiFeatures: mt2.aiFeatures,
    ...
    if (msg) {
          setSubmitProgress(6, 6, "Envío completado ✅");
